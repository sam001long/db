...
- 針對 Xbot/Mixamo 骨頭命#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
把 db/measurements.csv 轉成 three.js AnimationClip JSON（docs/animation/motions/*.json）
支援輸入格式：
 A) 長表：timestamp, joint, metric(=angle_x/angle_y/angle_z 或 x/y/z/ax/ay/az/angle/rot_x/rot_y/rot_z), value[, unit]
 B) 寬表：timestamp, RightArm_x/RightArm_y/RightArm_z ...
 C) 單一角度欄：timestamp, joint, angle_deg|angle_rad|theta|angle[, axis=x|y|z]（預設 y）

針對 Xbot/Mixamo 命名；找不到骨頭或資料不完整會略過該 track。
"""
...
def assemble_long_format(df: pd.DataFrame) -> Dict[str, Dict[str, List[float]]]:
    ...
    def norm(m: str):
        ...
        if m in ("y","ay","angle_y","rot_y","angle"): return "y"  # 'angle' 預設當 y 軸
...
def assemble_single_angle(df: pd.DataFrame) -> Dict[str, Dict[str, List[float]]]:
    # 支援 angle_deg / angle_rad / theta / angle
    angle_col = None
    unit_hint = None
    ...
    if angle_col == "angle_deg":
        unit_hint = "deg"
    elif angle_col in ("angle_rad","theta"):
        unit_hint = "rad"
    else:
        # 看 unit 欄，沒有就當 deg
        unit_hint = "rad" if ("unit" in df.columns and df["unit"].astype(str).str.contains("rad", case=False, na=False).any()) else "deg"
    ...
    for (j, ax), g in df2.groupby(["joint","axis"]):
        ...
        if ax.startswith("x"):
            rec["x"] = vals
        elif ax.startswith("z"):
            rec["z"] = vals
        else:
            rec["y"] = vals
        # 補齊長度
        n = min(len(rec["times"]), len(rec["x"]), len(rec["y"]), len(rec["z"]))
        for k in ("times","x","y","z"):
            rec[k] = rec[k][:n]
        # 把單位放進去方便 build_clip 用（選擇其一）
...
def build_clip(tracks_xyz: Dict[str, Dict[str, List[float]]], unit_hint: str = "deg") -> Dict[str, Any]:
    ...
    for ck, rec in tracks_xyz.items():
        ...
        # 單位偵測（單一角度欄可能放在 rec 裡）
        hint = rec.get("_unit_hint", unit_hint)
        if str(hint).lower().startswith("deg"):
...
def main():
    ...
    if not tracks:
        tracks = assemble_single_angle(df)
    if not tracks:
        raise RuntimeError("辨識不到角度欄位：\nA) 需 metric(angle_x/y/z) + value，或 axis + value\nB) 或 *_x/_y/_z 寬表\nC) 或 angle_deg/angle_rad/theta/angle（可加 axis）")
